...
class NRMysql2Client < Mysql2::Client
  ...
  def query(sql, *args)
    ...
    table = sql[/\bisu|isu_condition|isu_trend|user|isu_association_config\b/] || 'other'
  ...
  class App < Sinatra::Base
    ...
    helpers do
      ...
      def user_id_from_session
        ...
        count = db.xquery('SELECT jia_user_id FROM `user` WHERE `jia_user_id` = ? limit 1', jia_user_id).first
      ...
      def calculate_condition_level(condition)
        return cache_condition[condition] if cache_condition[condition]
        ...
        cache_condition[condition] = result
      end
    ...
    get '/api/isu' do
      ...
      # response_list = db_transaction do
        ...
        uuids = isu_list.map { |isu| isu.fetch(:jia_isu_uuid) }
        value = "'" + uuids.map(&:to_s).join("','") + "'"
        last_conditions = db.query("SELECT * FROM `isu_trend` WHERE `jia_isu_uuid` IN (#{value})")
        last_conditions_per_uuid = last_conditions.group_by { |cond| cond.fetch(:jia_isu_uuid) }

        response_list = isu_list.map do |isu|
          last_condition = last_conditions_per_uuid[isu.fetch(:jia_isu_uuid)]&.first
    ...
    get '/api/isu/:jia_isu_uuid/icon' do
      ...
      jia_isu_uuid = params[:jia_isu_uuid]
      isu = db.xquery('SELECT `image` FROM `isu` WHERE `jia_user_id` = ? AND `jia_isu_uuid` = ? LIMIT 1', jia_user_id, jia_isu_uuid).first
      halt_error 404, 'not found: isu' unless isu
    ...
    get '/api/isu/:jia_isu_uuid/graph' do
      ...
      cnt = db.xquery('SELECT id AS `cnt` FROM `isu` WHERE `jia_user_id` = ? AND `jia_isu_uuid` = ? LIMIT 1', jia_user_id, jia_isu_uuid).first
    ...
    def generate_isu_graph_response(jia_isu_uuid, graph_date)
      date_str = graph_date.strftime('%F')
      rows = db.xquery('SELECT * FROM `isu_condition` WHERE `jia_isu_uuid` = ? AND timestamp > ? AND timestamp <= ? ORDER BY `timestamp` ASC', jia_isu_uuid, date_str + ' 00:00:00', date_str + ' 23:59:59')
    ...
    get '/api/trend' do
      trend_data = db.query('SELECT isu.id AS `id`,
                              isu.character AS `character`,
                              isu.jia_isu_uuid AS `jia_isu_uuid`,
                              isu_trend.condition AS `condition`,
                              isu_trend.timestamp AS `timestamp`
                              FROM `isu_trend`
                              INNER JOIN `isu` on isu.jia_isu_uuid = isu_trend.jia_isu_uuid')
      trend_data_per_char = trend_data.group_by { |r| r.fetch(:character) }

      res = trend_data_per_char.keys.map do |character|
        isu_list = trend_data_per_char[character]
        ...
        isu_list.each do |isu|
          ...
          trend_condition = { isu_id: isu.fetch(:id), timestamp: isu.fetch(:timestamp).to_i }
    ...
    post '/api/condition/:jia_isu_uuid' do
      ...
      halt_error 400, 'bad request body' if json_params.empty?
        ...
        row = db.xquery('SELECT id FROM `isu` WHERE `jia_isu_uuid` = ? LIMIT 1', jia_isu_uuid).first
        halt_error 404, 'not found: isu' if row.nil?

        json_params.sort_by! { |cond| cond.fetch(:timestamp) }.reverse

        values = []
        trends = []
        json_params.each do |cond|
          timestamp = Time.at(cond.fetch(:timestamp)).strftime('%F %T')
          halt_error 400, 'bad request body' unless valid_condition_format?(cond.fetch(:condition))

          row = [jia_isu_uuid, timestamp, (cond.fetch(:is_sitting) ? 1 : 0), cond.fetch(:condition), cond.fetch(:message)]
          values << ("('" + row.map(&:to_s).join("', '") +"')")
        end

      db_transaction do
        db.query(
          "INSERT INTO `isu_condition` (`jia_isu_uuid`, `timestamp`, `is_sitting`, `condition`, `message`) VALUES #{values.join(', ')}"
        )
        db.query(
          "INSERT INTO `isu_trend` (`jia_isu_uuid`, `timestamp`, `is_sitting`, `condition`, `message`) VALUES #{values.last} ON DUPLICATE KEY UPDATE timestamp=VALUES(timestamp), `is_sitting`=VALUES(`is_sitting`), `condition`=VALUES(`condition`), `message`=VALUES(`message`)"
        )
      end
